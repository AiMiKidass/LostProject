...
/**
 * 重写日志:
 * 1.修复了布局名相同时,ResId错误重复的问题
 * 2.修复了可以通过返回键到内存卡根目录的问题 目前只能看SD卡根目录
 * 3.修复了在切换成多选模式时,已禁用的排序按钮显示的问题(menu)
 * 4.修复了--- 修复多选模式后退返回阅览模式 禁用菜单效果失效的问题
 * 5.禁用视图切换
 */
public class ExFilePickerActivity extends AppCompatActivity implements OnListItemClickListener,
    ...
    private static final String TOP_DIRECTORY2 = "/";
    ...
    private void initSDcardPath() {
        if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
            TOP_DIRECTORY = Environment.getExternalStorageDirectory().getPath();
        }
    }
    ...
    public void onListItemLongClick(int position) {
        if (!mIsMultiChoiceModeEnabled && position != OnListItemClickListener.POSITION_UP) {
            ...
            // 根据设置决定是否禁用按钮
            setAllViewsVisible(mToolbar.getMenu());
        }
    }
    ...
    private void setupViews() {
        ...
        setAllViewsVisible(menu);
    }

    private void setAllViewsVisible(Menu menu) {
        if (!checkNull(menu.findItem(R.id.new_folder))) {
            menu.findItem(R.id.new_folder).setVisible(!mIsNewFolderButtonDisabled);
        }

        if (!checkNull(menu.findItem(R.id.sort))) {
            menu.findItem(R.id.sort).setVisible(!mIsSortButtonDisabled);
        }

        if (!checkNull(menu.findItem(R.id.invert_selection))) {
            menu.findItem(R.id.invert_selection).setVisible(false);
        }

        if (!checkNull(menu.findItem(R.id.change_view))) {
            menu.findItem(R.id.change_view).setVisible(false);
        }
    }

    private boolean checkNull(MenuItem item) {
        return item == null;
    }

package com.example.alex.newtestproject.rxJava.fragment.bean;

import com.example.alex.newtestproject.rxJava.fragment.bean.BaseBean;

import java.io.Serializable;

/**
 * 登录bean
 *
 * @author Administrator
 *
 */
public class LoginVo extends BaseBean implements Serializable{
    public UserInfo obj;// 返回信息的实体类

    public class UserInfo implements Serializable{
        public String userId; // 用户ID
        public String token;
        public int num;// 如果登录失败，会记录次数，如果次数大于3，就要输入验证码
        public int realNameOrNot;//是否实名
        public String realName; // 真实姓名
        public String idNo;//身份证号
        public String mobilePhone; // 用户手机号
        public int mobileStatus; // 手机是否验证
        public String email ; // 邮箱
        public int emailStatus; // 邮箱是否验证
        public int userType; // 用户类型 １、普通用户，２、企业用户，３、系统账户
        public int superUser; // 是否为超级用户0:普通用户，1：超级用户，2：中间用户
        public Cardata bankCard; // 绑定的银行卡信息
        public int userAccountType;//用户帐户类型1, "个人基本户"2, "保证金户"3, "准备金"4, "存钱罐"5, "企业基本户"6, " 企业存钱罐"
        public int denyWithdrawRecharge;// 是否禁止充值提现(0:不禁止；1：禁止)
        public String denyWithdrawRechargeMsg ;// 禁止充值提现返回方案(如:春节期间,暂停充值提现)
        public int payMethod;//是否开通委托划扣（ 0：已授权  1：未开通）

        public int hasJrdProduct;// 是否已经选择佳人贷套餐（1已选择 0 未选择）
        public int isUpdateJrd;// 是否可以修改佳人贷套餐（1可修改 0 不可修改）

    }

    public class Cardata implements Serializable{
        /** "ICBC" 银行编码 */
        public String bankCode;
        /** "中国工商银行" */
        public String bankName;
        /** "36214" 新浪银行卡号 */
        public String cardId;
        /** "6222000200103824280" 银行卡号 */
        public String cardNo;
        /** 961 */
        public String id;
        /** 1 是否快捷卡 */
        public String isDefault;
        /** "15910954033" 绑定的手机号 */
        public String phoneNo;
        /** "吴桂仙" 持卡人 */
        public String realName;
        /** 今日已充金额 */
        public double hasRechargeToday;
        /** double 单日限额 */
        public double dailyLimit;
        /** double 单日单笔限额 */
        public double singleDayLimit;
        /** double 单笔最低支付限额 */
        public double singleMinPayLimit;
        /** double 首次绑卡充值限额 */
        public double firstBindingPay;
        /** double 提现单日限额 */
        public double withdrawDailyLimit;
        /** double 提现单笔限额 */
        public double singleWithdrawLimit;

        @Override
        public String toString() {
            return "Cardata [bankCode=" + bankCode + ", bankName=" + bankName + ", cardId=" + cardId + ", cardNo="
                    + cardNo + ", userId=" + id + ", isDefault=" + isDefault + ", phoneNo=" + phoneNo + ", realName="
                    + realName + ", hasRechargeToday=" + hasRechargeToday + "]";
        }

    }


}

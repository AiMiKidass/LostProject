...
/**
 * 工具类 控制日志输出，生产环境不打印日志，调试状态可打印日志
 */
public class LogUtils {

    private static String TAG = "NewTestProject";
    ...
    /**
     * 注:如果不要打印某一类log,那么只要把LOG_LEVEL值调到与其相等就可以了.当LOG_LEVEL为0时全部不打印 1时只打印toast
     * 测试用10   正式用0
     */
    ...
    public static void d(String msg) {
        if (LOG_LEVEL > VERBOSE) {
            Log.d(TAG, ifMsgNull(msg));
        }
    }

    private static String ifMsgNull(String msg) {
        if (msg == null) {
            return "";
        }
        return msg;

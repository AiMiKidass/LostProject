...
public class FileUtil {
    ...
    private final int MAX_TASK_SIZE = 5;

    public FileUtil() {
        threadPool = new ThreadPoolExecutor(MAX_TASK_SIZE,
                MAX_TASK_SIZE * 3, 1, TimeUnit.SECONDS,
                new ArrayBlockingQueue<Runnable>(MAX_TASK_SIZE * 2));
    }
     ...
     * @param cacheDirectory 指定缓存写入目录
     * @param byteSize       按多少字节大小拆分
     * @return 拆分后的文件名列表
     * @throws IOException
     */
    @SuppressWarnings("RedundantThrows")
    public List<String> splitBySize(String fileName, String cacheDirectory, int byteSize)
            throws IOException {
        ...
        for (int i = 0; i < count; i++) {
            String partFileName = cacheDirectory + file.getName() + "."
     ...
     */
    @SuppressWarnings("RedundantThrows")
    public List<String> splitBySize(String fileName, String cacheDirectory, int byteSize, int count)
            throws IOException {
        List<String> parts = new ArrayList<>();
        File file = new File(fileName);
        int countLen = (count + "").length();
    ...
    public void mergePartFiles(String dirPath, String partFileSuffix,
        ...
        ArrayList<File> partFiles = getDirFiles(dirPath,
                partFileSuffix);

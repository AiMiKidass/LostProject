package com.example.alex.newtestproject.model.custom;
 ...
 * ================================================
 */
public class CustomUploadTask<T> implements Runnable {
    ...
    public CustomUploadTask(String tag, Request<T, ? extends Request> request) {
    ...
    public CustomUploadTask<T> priority(int priority) {
        progress.priority = priority;
        return this;
    }

    public CustomUploadTask<T> extra1(Serializable extra1) {
        progress.extra1 = extra1;
        return this;
    }

    public CustomUploadTask<T> extra2(Serializable extra2) {
        progress.extra2 = extra2;
        return this;
    }

    public CustomUploadTask<T> extra3(Serializable extra3) {
        progress.extra3 = extra3;
        return this;
    }

    public CustomUploadTask<T> save() {
        UploadManager.getInstance().replace(progress);
        return this;
    }

    public CustomUploadTask<T> register(UploadListener<T> listener) {
        if (listener != null) {
    ...
    /** 删除一个任务,会删除下载文件 */
    public com.lzy.okserver.upload.UploadTask<T> remove() {
        pause();
        UploadManager.getInstance().delete(progress.tag);
        //noinspection unchecked
        com.lzy.okserver.upload.UploadTask<T> task = (com.lzy.okserver.upload.UploadTask<T>) OkUpload.getInstance().removeTask(progress.tag);

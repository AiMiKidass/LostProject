...
public class DrivingRouteOverLay extends RouteOverlay {
    ...
    public void addToMap() {
        ...
        try {
            ...
            tmcs = new ArrayList<>();
            List<DriveStep> drivePathSteps = drivePath.getSteps();
            mPolylineOptions.add(startPoint);

            for (DriveStep step : drivePathSteps) {
                List<LatLonPoint> latlonPoints = step.getPolyline();
                List<TMC> tmclist = step.getTMCs();
                tmcs.addAll(tmclist);
                addDrivingStationMarkers(step, AMapUtil.convertToLatLng(latlonPoints.get(0)));
        ...
        for (int i = 0; i < tmcSection.size(); i++) {
            ...
            if (i == tmcSection.size() - 1 && mPolylineOptionscolor != null) {
                ...
                addPolyLine(new PolylineOptions()
                        .add(AMapUtil.convertToLatLng(mployline.get(mployline.size() - 1)), endPoint)
                        .setDottedLine(true));
            }
        }
    }
    ...
    public void setIsColorfulline(boolean iscolorfullline) {

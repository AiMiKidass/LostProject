...
import com.example.alex.newtestproject.gaode.adapter.BusResultListAdapter;
import com.example.alex.newtestproject.gaode.util.AMapUtil;
import com.example.alex.newtestproject.utils.MapToastUtil;


public class RoutePlanningActivity extends Activity implements OnMapClickListener,
        OnMarkerClickListener, OnInfoWindowClickListener, InfoWindowAdapter, OnRouteSearchListener {
    private static final java.lang.String INTENT_KEY_STARTPOINT = "intent_key_startpoint";
    private static final java.lang.String INTENT_KEY_ENDPOINT = "intent_key_endpoint";
    private AMap aMap;
...
//		getIntentData();
     ...
     * 获取起点和终点的point
     */
    private void getPointFromIntent() {
        Intent intent = getIntent();

        // 当前地点和终点
        LatLng startPoint = intent.getParcelableExtra(INTENT_KEY_STARTPOINT);
        LatLng endPoint = intent.getParcelableExtra(INTENT_KEY_ENDPOINT);
        intent.putExtra("", mStartPoint);
    }
    ...
    public void searchRouteResult(int routeType, int mode) {
        if (mStartPoint == null) {
            MapToastUtil.show(mContext, "起点未设置");
            return;
        }
        if (mEndPoint == null) {
            MapToastUtil.show(mContext, "终点未设置");
        }
        showProgressDialog();
        ...
        if (errorCode == AMapException.CODE_AMAP_SUCCESS) {
            if (result != null && result.getPaths() != null) {
                if (result.getPaths().size() > 0) {
                    ...
                    if(mBusResultListAdapter == null){
                        mBusResultListAdapter = new BusResultListAdapter(mContext, mBusRouteResult);
                        mBusResultList.setAdapter(mBusResultListAdapter);
                        mBusResultList.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                            @Override
                            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                                final BusPath item = mBusRouteResult.getPaths().get(position);
                    ...
                    MapToastUtil.show(mContext, R.string.no_result);
                }
            } else {
                MapToastUtil.show(mContext, R.string.no_result);
            }
